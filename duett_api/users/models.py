...
class AgencyManagedUser(TimestampMixin):
    ...
    managed_user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, related_name="managed_user", db_constraint=False
    )
...
class UserPreferences(TimestampMixin):
    user = models.OneToOneField(
        User, on_delete=models.DO_NOTHING, db_constraint=False)

    # comma separated list of column ids
    request_table_columns = models.CharField(
